...
public class FurnitureManagerUI : MonoBehaviour
{
    ...
    public void Start()
    {
        var validFurniture = database.GetRecipesForCategory(Categories.FurnitureType.MACHINES);
        int currentLevel = ExperienceManager.Instance.CurrentLevel;

        foreach (Transform child in contentHolder)
            Destroy(child.gameObject);

        for (int i = 0; i < validFurniture.Count; i++)
        {
            var menuItem = Instantiate(MenuItemGO, contentHolder.transform);
            menuItem.GetComponent<MenuFurnitureItem>().SetupMenuItem(this, validFurniture[i], currentLevel);
    ...
    public void SelectItem(PlaceableData furniture)
    {
        //RoomEditorManager.Instance.StartPlacing(furniture);
